...
//##############################################################################
    ...
    */
    //--------------------------------------------------------------------------
    CMercury::CMercury(GLdouble const akdPos[3], GLdouble const akdForward[3], 
    ...
    //----------------------------------------------------------------------
    /**
    * Minimal specification constructor for a stationary CMercury.
    *
    * @param akdPos The position in cartesian co-ordinates.
    */
    //----------------------------------------------------------------------
    CMercury::CMercury(GLdouble const akdPos[3]) : CHeavenlyBody(akdPos) {
        CommonConstructor();
    }
    ...
    */
    //----------------------------------------------------------------------
    void CMercury::CommonConstructor(void) {
        //Read these from XML
        //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        m_nDamage = 10000000;
    ...
    */
    //----------------------------------------------------------------------
    void CMercury::Init(void) {
        glNewList(gknAliveDisplay, GL_COMPILE);
	
            CTexture texture(400,"mercury.bmp");
            texture.Select();		 
            GLUquadricObj *q;
            q = gluNewQuadric();
        ...
        glNewList(gknDyingDisplay, GL_COMPILE);
            ...
            CTexture texture2(401,"mercury.bmp");
            texture2.Select();		 
            GLUquadricObj *w;
            w = gluNewQuadric();
